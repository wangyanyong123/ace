 ...
 */
@Service
public class BizReservationPersonBiz extends BusinessBiz<BizReservationPersonMapper,BizReservationPerson> {
    private Logger log = LoggerFactory.getLogger(BizReservationPersonBiz.class);
    @Autowired
    ...
    public ObjectRestResponse<List<ReservationVo>> getReservationList(String projectId, String classifyId,
                                                                      Integer page, Integer limit,List<String> cityCodeList){
        ...
        List<ReservationVo> reservationVoList = bizReservationPersonMapper
                .selectReservationByClassifyId(projectId, classifyId, startIndex, limit,cityCodeList);
...
//            reservationInfos.setBuyNum("0");
            ...
            if(productSpecInfoList != null && productSpecInfoList.size() > 0){
                for(ProductSpecInfo info : productSpecInfoList){
                    ...
                    info.setSpecImageList(imglist);
                    info.setPrice(df.format(Double.parseDouble(info.getPrice())));
                    List<CacheStoreQuery> cacheStoreQueryList = storeReservationFegin.getSaleStore(info.getId());
                    cacheStoreQueryList.forEach(cacheStoreQuery -> {
                        Integer timeSlot =  DateUtils.isAmOrPm(new Date());
                        if(timeSlot.equals(cacheStoreQuery.getTimeSlot())){
                            info.setIsLimit(Boolean.FALSE);
                            info.setStockNum(cacheStoreQuery.getCacheStore().getStoreNum());
                        }
                    });
